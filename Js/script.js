...
mainAudio.addEventListener("timeupdate",(e)=>{
    ...
    const duration=e.target.duration;//getting tot duration of song
...
//above we just changed the icon, now lets work on what to do after the song ended

mainAudio.addEventListener("ended",()=>{
   //we will do according to the icon means if user has set icon to loop song then we will
   //repeat the current song and will do further accordingly
   let getText=repeatBtn.innerText;

    //lets do different changes on different icon click using switch
    switch(getText)
    {
        case "repeat"://if this icon is repeat then simply we call the nextMusic() 
        //so the next song will play 
            nextMusic();
            break;
        case "repeat_one"://if icon is repeat_one then we'll change the current
        //playing song currentTime to 0. so song will play from beginning
        ...
        //generating random index between the man range of array length
            ...
            } while(musicIndex==randIndex);//this loop run until the nexxt random  number won't be the same of current music index
...
for(let i=0;i<allMusic.length;i++)
{
    ...
    liAudioTag.addEventListener("loadeddata",()=>{
        ...
        //adding t duraion attribute which we'll use below
    ...
    for(let j=0;j<allLiTags.length;j++){
           let audioTag=allLiTags[j].querySelector(".audio-duration");
            ...
            if(allLiTags[j].classList.contains("playing"))
            {
                ...
                audioTag.innerText=adDuration;//passing t-duraiton value to audio duration innerText
            }
